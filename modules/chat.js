// ========== modules/chat.js · 2026-06-09 最终版 ==========
(function() {
    ...
    function replacePlaceholders(text) {
        const userName = localStorage.getItem('myName') || '我';
        const nickname1 = localStorage.getItem('myNickname1') || '';
        const nickname2 = localStorage.getItem('myNickname2') || '';
        const displayNickname = nickname1 || nickname2 || userName;
        return text.replace(/\{\{用户名\}\}/g, userName).replace(/\{\{昵称\}\}/g, displayNickname);
    }
    ...
    function renderChat() {
        ...
        for (let i = 0; i < chatMessages.length; i++) {
            ...
            if (msg.imgSrc) bubble.innerHTML += '<img src="' + msg.imgSrc + '" style="max-width:150px;border-radius:12px;">';
    ...
    function addMessage(text, isMe, imgSrc, options) {
        const isSticker = options && options.isSticker;
        let finalText = text;
        if (!isMe) {
            finalText = replacePlaceholders(text);
        }
        chatMessages.push({ text: finalText, time: new Date().toLocaleTimeString(), isMe: isMe, imgSrc: imgSrc || null, isSticker: isSticker || false });
        saveChatData();
        renderChat();
    }
